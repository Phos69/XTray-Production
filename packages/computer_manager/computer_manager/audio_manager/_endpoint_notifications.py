...
# Retired callbacks are intentionally kept alive for the process lifetime.
# Windows can still enter a native callback thunk briefly after unregistering;
# if ctypes has already freed that thunk, the process dies without a Python
# traceback. The callback state is deactivated first, so stale calls return.
_RETIRED_CALLBACKS: list[tuple[Any, Any, ctypes.c_void_p | None, Any]] = []
_ORPHANED_CALLBACKS = _RETIRED_CALLBACKS
...
class EndpointChangeListener:
    ...
    def _teardown_locked(self) -> None:
        ...
        if callback_pointer is not None:
            _RETIRED_CALLBACKS.append(
                (self._vtable, self._callback_object, callback_pointer, callback_state)
            )
        if callback_pointer is not None and not unregistered:
            self._log.warning(
