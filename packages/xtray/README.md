...
pip install -e .\packages\xtray_sync
```
...
- LAN sync is provided by the sibling `xtray_sync` package and is included by
  the `full`, `build`, and `dev` extras.
...
xtray config token
xtray sync status
xtray diagnostics tail --lines 120
