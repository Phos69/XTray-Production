...
from .dialogs import TrayOptionsDialog
...
class TrayApp:  # pragma: no cover - imported only when GUI dependencies exist
    def __init__(self, app: Any) -> None:
        ...
        self._native_audio_events_disabled = _env_truthy(
            os.environ.get(_DISABLE_NATIVE_AUDIO_EVENTS_ENV)
        )
        if self._native_audio_events_disabled:
            app_logging.get_logger("tray").warning(
                "native audio event listeners disabled by %s",
                _DISABLE_NATIVE_AUDIO_EVENTS_ENV,
            )
        ...
        # so the call succeeds and the deferred refresh fires after
        # _register_live_data_sources has run.
    ...
    def _download_and_install_update(self, update_info: updater.UpdateInfo) -> None:
        if self._update_install_in_progress:
            return
        ...
        def run_install() -> Path:
            ...
            return updater.start_update_installer(installer)
    ...
    def _run_in_thread(
        ...
        self._threads.run(target, on_success, on_failure, cleanup)
