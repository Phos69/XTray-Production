...
INSTALLER_TASK_PC_SYNC = "pcsync"
INSTALLER_TASK_NETWORK_MANAGER_FULL = "network_manager_full"
...
@dataclass(frozen=True)
class UpdateInstallOptions:
    pc_sync: bool = False
    network_manager_full: bool = False
...
def releases_api_url() -> str:
    override = os.environ.get(ENV_UPDATE_API_URL)
    if override:
        return override.removesuffix("/latest")
    repo = (os.environ.get(ENV_UPDATE_REPO) or DEFAULT_UPDATE_REPO).strip()
...
def _version_key(value: str) -> tuple[tuple[int, ...], int, int]:
    ...
    prerelease_match = re.search(
        r"(?P<label>a|alpha|b|beta|rc)(?P<num>\d*)$",
        text,
        re.IGNORECASE,
    )
    stage_rank = 3
    ...
    if prerelease_match is not None:
        label = prerelease_match.group("label").casefold()
        stage_rank = {"a": 0, "alpha": 0, "b": 1, "beta": 1, "rc": 2}[label]
        prerelease_number = int(prerelease_match.group("num") or 0)
        text = text[: prerelease_match.start()]
    parts = [int(part) for part in re.findall(r"\d+", text)]
    while len(parts) > 1 and parts[-1] == 0:
        parts.pop()
    return tuple(parts or [0]), stage_rank, prerelease_number
...
def update_from_release_payload(
    ...
    *,
    ...
    if payload.get("draft") or (payload.get("prerelease") and not include_prereleases):
        return None
...
def _load_releases_url(
    ...
    *,
    ...
    try:
        ...
        if exc.code == 404:
            raise UpdateError("no public XTray releases were found") from exc
        raise UpdateError(f"GitHub update check failed: HTTP {exc.code}") from exc
    except URLError as exc:
        reason = getattr(exc, "reason", exc)
        raise UpdateError(f"GitHub update check failed: {reason}") from exc
    ...
    if not isinstance(payload, list):
        raise UpdateError("GitHub releases response had an unexpected shape")
    return [item for item in payload if isinstance(item, dict)]


def update_from_releases_payload(
    ...
    *,
    ...
    missing_asset_errors: list[UpdateError] = []
    for release in payload:
        try:
            update = update_from_release_payload(
                release,
                current_version=current_version,
                include_prereleases=include_prereleases,
            )
        except UpdateError as exc:
            missing_asset_errors.append(exc)
            continue
        if update is not None:
            candidates.append(update)
    if candidates:
        return max(candidates, key=lambda item: _version_key(item.version))
    if missing_asset_errors:
        raise missing_asset_errors[0]
    return None


def available_updates_from_releases_payload(
    payload: list[dict[str, Any]],
    *,
    current_version: str = __version__,
    include_prereleases: bool = False,
...
def list_available_updates(
    *,
    ...
    payload = _load_releases_url(releases_api_url(), timeout=timeout)
    return available_updates_from_releases_payload(
        payload,
        current_version=current_version,
        include_prereleases=include_prereleases,
    )
...
def check_for_update(
    *,
    ...
    if include_prereleases:
        payload = _load_releases_url(releases_api_url(), timeout=timeout)
        return update_from_releases_payload(
            payload,
            current_version=current_version,
...
def installer_args_for_options(
    options: UpdateInstallOptions | None = None,
    *,
    base_args: tuple[str, ...] = DEFAULT_INSTALLER_ARGS,
) -> tuple[str, ...]:
    options = options or UpdateInstallOptions()
    tasks: list[str] = []
    if options.pc_sync:
        tasks.append(INSTALLER_TASK_PC_SYNC)
    if options.network_manager_full:
        tasks.append(INSTALLER_TASK_NETWORK_MANAGER_FULL)
    if not tasks:
        return base_args
    return (*base_args, f"/MERGETASKS={','.join(tasks)}")
