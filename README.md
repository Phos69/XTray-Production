...
Richiede Windows, Python 3.11+ e .NET 8 SDK.
...
.\.venv\Scripts\python.exe -m pip install `
  ...
  -e .\packages\network_manager `
  -e .\packages\xtray_sync `
  -e .\packages\xtray[full]

dotnet build XTray.sln
.\run-tray.bat
```
...
- `XTRAY_ENABLE_NATIVE_AUDIO_EVENTS=1`: riabilita gli event listener audio
  nativi Windows. Di default XTray usa il polling del volume, piu' stabile
  con alcuni driver audio.
- `XTRAY_DISABLE_NATIVE_AUDIO_EVENTS=1`: forza il polling anche se gli event
  listener nativi sono stati abilitati.
...
- Importa da un altro PC XTray sulla LAN impostazioni Network Manager,
  dispositivi, password router/switch e profili display.
...
- Abilita `LAN Sync` dalle Options su ogni PC e imposta la stessa password.
- XTray annuncia solo metadati non sensibili sulla LAN; i dati vengono esportati
  solo dopo handshake con password e payload cifrato.
- L'import e' manuale: dal menu tray usa `Import from LAN PC...`, scegli il PC
  sorgente e conferma le sezioni da importare.
- Le impostazioni Network Manager e la rubrica dispositivi importate sostituiscono
  quelle locali, creando prima un backup in `%APPDATA%\XTray\sync_backups\`.
- Le password sincronizzate sono solo quelle router/switch di Network Manager.
  MQTT, token Home Assistant e API token restano locali.
- I profili display importati sovrascrivono quelli locali con lo stesso nome.
...
- La password LAN Sync usa Windows Credential Manager con servizio `XTray Sync`
  e non viene scritta in `settings.json`.
